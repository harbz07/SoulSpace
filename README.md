...
MindBridge is an LLM router component that lets Claude.ai, ChatGPT, Gemini, and Mistral call any configured model for second opinions and multi-model workflows.

- **MCP server** (stdio) — for Claude Desktop, Cursor, Windsurf
- **HTTP server** — for web AI integrations (Claude.ai, ChatGPT Actions, Gemini Extensions)
- **Cloudflare Worker** — serverless deployment for public access
...
See `mindbridge/README.md` and `mindbridge/REMOTE_SETUP.md` for full documentation.
...
3. **Get Channel IDs**
   ...
   - Paste the IDs into `.env`:
     ...
     - `CHANNEL_THE_MIRROR` - Status updates
     - `CHANNEL_THE_COUNSEL` - Commands/control
>>>>>>> 488c328131a58643b1e0be0e205bd617e05190d2
...
<<<<<<< HEAD
- **`/trace `** - Get raw logs for specific trace ID

### Code Execution (⚠️ Single-user system!)
- **`/exec `** - Execute Python code (30s timeout)
- **`/shell `** - Execute shell command (30s timeout, safety checks)

### Authentication
- **`/auth `** - Authenticate service (gmail, calendar)

### Data Management
- **`/export`** - Generate full data dump for backup
- **`/purge `** - Delete specific memory
=======
